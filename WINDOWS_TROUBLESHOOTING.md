...
**Note:** As of the latest configuration, clangd is configured with `--query-driver` flag which automatically queries GCC for its system headers. If you have MinGW installed, clangd should automatically detect the headers. The manual configuration below is only needed for special cases.
